...
description: 跨境电商上架内容模块化生产 + 图生图产品图 + 内嵌 QA + 飞书云文档（Docx）交付（v2.6.1）。
version: 2.6.1
---
...
2. **空格 → 横线**：这是唯一强制的字符转换（保证 URL / shell 友好）。
3. **删除文件系统禁用字符**：`/` `\\` `:` `*` `?` `"` `<` `>` `|` 直接删除。**其他字符全部保留**（下划线、加号、点、括号、感叹号、$ 号、# 号、Đ、é、ệ 等等一律保留）。
4. **连续横线折叠**为单个横线；**首尾横线剥离**。
5. **附加国家代码**：ISO 3166-1 alpha-2 **大写**（VN / US / TH / BR / JP / KR / DE / FR …），用横线拼接在品牌型号之后。
6. **不接受用户自定义规则**；只接受用户改"从这段话里抽出的品牌/型号短语"与国家代码。
