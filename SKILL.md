...
description: 跨境电商上架内容模块化生产 + 图生图产品图 + 内嵌 QA + 飞书云文档（Docx）交付（v2.5）。
version: 2.5.0
---
...
### 步骤 5：交付（内嵌 Post-QA + 分渠道分发）

- Agent 自己对每张生成图做多模态自审（第 14 章），产出 Post-QA 报告。
- Post-QA 只剔除"明显不是产品图"（生成失败/损坏/严重跑题/触发过滤器占位图），有瑕疵但产品可辨认的**全部保留发出**。
- 交付形态按当前渠道分流：
  - **飞书渠道 + `lark-cli` 已认证** → 走第 18 章「飞书云文档输出模式」（Docx + 云盘图片 + 图文卡片 + 聊天框链接消息）。
  - **飞书渠道 + `lark-cli` 未认证 + 用户拒绝授权**，或**非飞书渠道** → 走第 16 章旧的 `feishu-tools send-card` 图文卡片路径（或渠道自有交付）。
...
每张图的提示词必须由三段组成（形态锁定 + 产品外观描述 + 构图/文字/背景）；当本图涉及多个 SKU/颜色变体时**必须**多加一段 Variant-Preservation Block：
...
**引号降降（强制约束）**：prompt 字符串禁止使用 ASCII 双引号 `"`（会破坏包含它的 batch JSON 或 shell 命令行）。如需强调使用中文双引号 `“”`、中文引号 `「」` 或 ASCII 单引号 `'`。QUALITY_GATE.md 会将它作为硬卡点。
...
- 🟡 图必须附"修复方式建议"四选一：**重出 / PS 后处理 / 遮盖裁剪 / 直接使用**。例：logo/文字错字 → PS；产品主体变形 → 重出；构图小瑕疵 → 遮盖裁剪；完全可接受 → 直接使用。
...
| 工具 | 用途 | 最低版本 | 仓库 | 安装命令 |
|---|---|---|---|---|
| `image-provider-gateway` | 图生图 / 文生图统一入口，OpenAI-compatible API；支持配置文件持久化 provider 凭据 + 结构化错误码 | `>= 0.2.0` | https://github.com/T-Chen-CN/image-provider-gateway | `uv tool install git+https://github.com/T-Chen-CN/image-provider-gateway` |
...
image-provider-gateway --version 2>/dev/null | grep -Eq " [0-9]+\.[0-9]+\.[0-9]+" || echo "ipg: NEEDS_UPGRADE"
...
- 长文本正文优先使用 `--text-file <path>` 传入 Markdown 文件；含 Markdown 代码块、反引号 `` ` ``、`$` 符号，或长度 > 500 字符时**必须**用 `--text-file`，避免 heredoc / 多行 shell 字符串导致解析错误。
...
- `YYYYMMDD` 为**首次为该 slug 建目录的日期**，后续复用时**不修改**。
- `slug` 由品牌 / 型号 / 类目自然语言生成（§18.4）。
...
2. 如果 `lark: AUTH_MISSING`：
   ...
   - 用户回"否" → 退回第 16 章图文卡片路径，本次不建 Docx。
...
从用户自然语言中提取品牌 + 型号后，agent 自行生成 slug 并让用户 confirm。规则：

- 支持中文、英文字母、数字、横线。
- 空格转横线；大写转小写。
- 去除所有非（中文 / 字母 / 数字 / 横线）字符。
- 连续横线折叠为单个横线；首尾横线剥离。
- 例：`ZGAR-001 Pro (X版)` → `zgar-001-pro-x版`；`兔女郎 Cosplay` → `兔女郎-cosplay`。
- **不接受用户自定义 slug 规则**；只接受用户改"从这段话里抽出的品牌/型号短语"。
...
| 文档 | `YYYYMMDD-{slug}-{批次:03d}.docx` | 每次跑 Skill 都 +1，无论改文字或改图 |

**图片位置**：`01`-`09` 对应 §7.4 的 9 图；SKU 图另起序列。

**图片选取（Docx 内嵌）**：`latest-per-slot`。对每个位置 `NN`，取该位置的**最大批次号图片**。举例：
```
文件夹里：Main001-01, Main001-02, Main002-03, Main001-04, ..., Main001-09
...
Docx 生成完毕后，Agent 在聊天框只发一条消息（**零文案输出**）：
