...
从用户自然语言中提取品牌 + 型号后，agent 自行生成 slug 并让用户 confirm。规则（**按顺序**）：

1. **英文字母转小写**（`ABC` → `abc`；中文与数字不变；中文本身无大小写概念）。
2. **空格 & 分隔类符号转横线**：包括空格、`_`、`+`、`.`、`/`、`\\`、`,`、`;`、`|`；这些符号不是被丢弃，而是转成分隔用的横线。
3. **删除其他非白名单字符**：白名单为「中文 / 英文字母 / 数字 / 横线」；括号、感叹号、$ 号、# 号等**全部丢弃**（不生成分隔）。
4. **连续横线折叠**为单个横线；**首尾横线剥离**。
5. **不接受用户自定义 slug 规则**；只接受用户改"从这段话里抽出的品牌/型号短语"。

例子：

| 输入 | → | 输出 |
|---|---|---|
| `ZGAR-001 Pro (X版)` | → | `zgar-001-pro-x版` |
| `兔女郎 Cosplay` | → | `兔女郎-cosplay` |
| `HELLO-世界-42` | → | `hello-世界-42` |
| `brand_name+v2.5` | → | `brand-name-v2-5` |
| `product/with/slashes` | → | `product-with-slashes` |
| `多个    空格` | → | `多个-空格` |
...
**顺序建议**：先发 Docx 与文件夹链接消息，再推 send-card 图文卡片，避免时序混乱。
