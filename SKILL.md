...
```bash
python3 scripts/run_manifest.py init ./run-manifest.json --market <market> --platform <platform> --category <category>
```
...
- 每图必须同时记录 `file_token` 与 `image_key`；卡片准备可与有序插图流水化，全部完成后统一执行 `validate --delivery`，再发送 Docx 链接与图文卡片。
