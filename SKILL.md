...
description: 跨境电商上架内容模块化生产 + 图生图产品图 + 内嵌 QA + 飞书云文档（Docx）交付（v2.6）。
version: 2.6.0
---
...
### 步骤 5：交付（v2.6 分形态 + 分渠道分流）

- 若涉及生图：Agent 自己对每张生成图做多模态自审（第 14 章），产出 Post-QA 报告。Post-QA 只剔除"明显不是产品图"（生成失败/损坏/严重跑题/触发过滤器占位图），有瑕疵但产品可辨认的**全部保留发出**。
- 分流决策（v2.6，先判定"是不是完整成品交付"，再按渠道分流）：

| 场景 | 交付形态 |
|---|---|
| 飞书渠道 + `lark-cli` 已认证 + 完整成品（完整文案 / 生图完成 / 完整=文案+图 / 组合模块完整交付） | 走 §18 Docx 交付 |
| 飞书渠道 + `lark-cli` 未认证 + 用户拒绝授权 | 走 §16 图文卡片（QA 报告作为文字块附上） |
| 非飞书渠道 | 走各渠道自有形态（IM 卡片 / 纯文本 / 附件） |
| 非完整成品（单模块 / 返工修订某一小节 / 用户显式指示"直接聊天框"） | 直接聊天框输出，不走 Docx |
